        ...
        if (isset($_POST) && !empty($_POST)) {
                ...
                if (isset($_POST['email']) && !empty($_POST['otp'])) {
                    ...
                    if (isset($checkmobilenootp) && !empty($checkmobilenootp)){
                       ...
                       redirect('https://supporthub.xytr.org/requester/reset_pass');
                    } else {
...
$name = $check_user1['c_name'];
  ...
  $username = !empty($check_user1['user_email']) ? $check_user1['user_email'] : '';
        ...
        if ($result_id) {
            redirect('https://supporthub.xytr.org/requester/ticketlist');
        }
    }
...
$name = $check_user1['c_name'];
  ...
  $username = !empty($check_user1['user_email']) ? $check_user1['user_email'] : '';
        ...
        if ($result_id) {
            redirect('https://supporthub.xytr.org/requester/welcome/replyview/'.$id);
        }
    }
		...
		if(isset($_POST) && !empty($_POST)){
			...
			if (isset($_POST['id']) &&  !empty($_POST['id'])){
				...
				//redirect('https://ezheal.ai/admin/appointlist')
			        ...
			        if ($result_id) {
            redirect('https://chauffit.com/ticket_admin/requesterlist');
        } 
		...
		if(isset($_POST) && !empty($_POST)){
			...
			if (isset($_POST['id']) &&  !empty($_POST['id'])){
				...
				//redirect('https://ezheal.ai/admin/appointlist')
			        ...
			        if ($result_id) {
            redirect('https://chauffit.com/ticket_admin/createuser');
        } 
		...
		if(isset($_POST) && !empty($_POST)){
			...
			if (isset($_POST['id']) &&  !empty($_POST['id'])){
				...
				//redirect('https://ezheal.ai/admin/appointlist')
			        ...
			        if ($result_id) {
            redirect('https://chauffit.com/ticket_admin/businessarea');
        } 
		...
		if(isset($_POST) && !empty($_POST)){
			...
			if (isset($_POST['id']) &&  !empty($_POST['id'])){
				...
				//redirect('https://ezheal.ai/admin/appointlist')
			        ...
			        if ($result_id) {
            redirect('https://chauffit.com/ticket_admin/rolepermission');
        } 
          ...
          $data = array();
			...
			if (isset($_POST['id']) &&  !empty($_POST['id'])){
				...
				//redirect('https://ezheal.ai/admin/appointlist')
			        ...
			        if ($result_id) {
            redirect('https://supporthub.xytr.org/requester/pageindex');
        } 
...
public function get_filtered_staff()
{
          ...
          //  print_r($chkreturnrequest);
            if ($chkreturnrequest) {
                if ($chkreturnrequest['status'] == 1) {
                  if ($chkreturnrequest['role'] == 'Requester'){
 ...
 $userdata = $this->session->userdata(USER_SESSION);
    ...
    if (!empty($subscriber_id)) {
     ...
     $whr1 = array('req_name'=>$subscriber_id);
...
public function exportuserreport(){ 
  ...
  public function deleteRecord()   
    ...
    public function download($fileName) {
        ...
        $filePath = FCPATH . "upload/channels/" . basename($fileName); // Use FCPATH for absolute path
        ...
        if (file_exists($filePath)) {
            ...
            show_404(); // Use CodeIgniter's 404 handler if the file is not found
        }
    }
...

	{
		...
		redirect("https://supporthub.xytr.org/requester/",'refresh');
